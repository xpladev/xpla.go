...
func parseCreateValidatorArgs(
	...
	if privKeyValAddr.String() != addr.String() {
		return nil, types.ErrWrap(types.ErrAccountNotMatch, "CreateValidatorMsg.ValidatorAddress and validator address generated by using private key are not same")
	}
	...
	if !ok {
		return nil, types.ErrWrap(types.ErrConvert, "wrong minSelfDelegation")
	}
...
func parseEditValidatorArgs(editValidatorMsg types.EditValidatorMsg, addr sdk.AccAddress) (stakingtypes.MsgEditValidator, error) {
	...
	if commisionRate != "" {
		...
		if err != nil {
			return stakingtypes.MsgEditValidator{}, types.ErrWrap(types.ErrConvert, err)
		}
		newRate = &rate
	}
	...
	if minSelfDelegation != "" {
		...
		if !ok {
			return stakingtypes.MsgEditValidator{}, types.ErrWrap(types.ErrConvert, "minimum self delegation must be a positive integer")
		}
...
func parseDelegateArgs(delegateMsg types.DelegateMsg, delAddr sdk.AccAddress) (stakingtypes.MsgDelegate, error) {
	...
	if err != nil {
		return stakingtypes.MsgDelegate{}, types.ErrWrap(types.ErrParse, err)
	}
...
func parseUnbondArgs(unbondMsg types.UnbondMsg, delAddr sdk.AccAddress) (stakingtypes.MsgUndelegate, error) {
	...
	if err != nil {
		return stakingtypes.MsgUndelegate{}, types.ErrWrap(types.ErrParse, err)
	}
...
func parseRedelegateArgs(redelegateMsg types.RedelegateMsg, delAddr sdk.AccAddress) (stakingtypes.MsgBeginRedelegate, error) {
	...
	if err != nil {
		return stakingtypes.MsgBeginRedelegate{}, types.ErrWrap(types.ErrParse, err)
	}
...
func parseHistoricalInfoArgs(historicalMsg types.HistoricalInfoMsg) (stakingtypes.QueryHistoricalInfoRequest, error) {
	...
	if err != nil {
		return stakingtypes.QueryHistoricalInfoRequest{}, types.ErrWrap(types.ErrParse, err)
	}
	if heightInt < 0 {
		return stakingtypes.QueryHistoricalInfoRequest{}, types.ErrWrap(types.ErrInvalidRequest, "height argument provided must be a non-negative-integer")
	}
...
func buildCommissionRates(rateStr, maxRateStr, maxChangeRateStr string) (commission stakingtypes.CommissionRates, err error) {
	if rateStr == "" || maxRateStr == "" || maxChangeRateStr == "" {
		return commission, types.ErrWrap(types.ErrInsufficientParams, "must specify all validator commission parameters")
	}
	rate, err := sdk.NewDecFromStr(rateStr)
	if err != nil {
		return commission, types.ErrWrap(types.ErrParse, err)
	}
...
func initializedNodeValidatorString(nodeKey string, privValKey string) (string, cryptotypes.PubKey, error) {
	...
	if err != nil {
		return "", nil, types.ErrWrap(types.ErrFailedToUnmarshal, "err reading privValidator key, ", err)
	}
